...
pub struct ProposalGenerator {
    ...
    max_failed_authors_to_store: u64,
...
impl ProposalGenerator {
    ...
    pub async fn generate_proposal(
        ...
        let failed_authors_for_current_block = self.compute_failed_authors(
            round,
        ...
        Ok(BlockData::new_proposal(
            payload,
            self.author,
            failed_authors_for_current_block,
            round,
            timestamp,
            quorum_cert,
        ))
    ...
    fn compute_failed_authors(
        ...
        let mut failed_authors_for_current_block = Vec::new();
        let mut i = previous_round + 1;
        if round - i > self.max_failed_authors_to_store {
            i = round - self.max_failed_authors_to_store;
        }
        while i < round {
            failed_authors_for_current_block.push((i, proposer_election.get_valid_proposer(i)));
            i += 1;
        }
